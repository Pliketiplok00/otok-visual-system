...
## ⚠️ DEVELOPER FAQ — IMPORTANT CLARIFICATIONS

### Q1: Backend/API Layer & Data Source?

**Current State:** Static mockup — all data is hardcoded in components.

**Production Plan:**
- **Backend:** Lovable Cloud (Supabase-based) — will provide PostgreSQL database, auth, storage, and edge functions
- **Auth Model:** Email/password authentication via Supabase Auth
- **Data Flow:** React Query for data fetching, Supabase client for CRUD operations
- **Mock Data:** Currently inline in components — will be migrated to database tables

**Tables Planned:**
| Table | Purpose |
|-------|---------|
| `events` | Calendar events with dates, locations, descriptions |
| `notifications` | Inbox messages for users |
| `schedules` | Ferry/bus schedules (sea & land routes) |
| `reports` | Click&Fix user-submitted issues |
| `feedback` | User suggestions and feedback |
| `flora_fauna` | Geopark species information |
| `wine_routes` | Wine route details and locations |

### Q2: Which Version is Production?

**PRODUCTION SCREENS:** All **V3** screens + **HomeV4**

| Screen Pattern | Status |
|---------------|--------|
| `*V3.tsx` | ✅ Production — final design |
| `HomeV4.tsx` | ✅ Production — final home layout |
| `*Alt.tsx` | ❌ Deleted — was prototype |
| `*.tsx` (without version) | ❌ Deleted — was legacy |

**Route Structure:** See `src/App.tsx` — only V3/V4 screens are routed.

### Q3: Automated Linting for Brutalist Rules?

**Current State:** Manual review only.

**Recommendation:** The following ESLint rules could be added but are NOT currently enforced:

```javascript
// Potential future eslint plugin (not implemented)
// Rules to consider adding via eslint-plugin-tailwindcss or custom:
rules: {
  // Warn on rounded classes
  'no-restricted-syntax': ['warn', {
    selector: 'Literal[value=/rounded/]',
    message: 'No rounded corners allowed in brutalist design'
  }]
}
```

**Manual Checklist (use during code review):**
- [ ] No `rounded-*` classes anywhere
- [ ] No `blur-*` or `backdrop-blur-*` classes
- [ ] No `shadow-*` with blur (use `shadow-brutal-*` utilities only)
- [ ] No `bg-gradient-*` classes
- [ ] All interactive elements have `border-[3px] border-foreground`
- [ ] All cards use `shadow-brutal` or color-specific shadow
- [ ] Touch targets minimum 44x44px

**Style Guide Reference:** `/style-guide` route shows all correct component usage.
